...
const Remove_pilot_role = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('/pilots/remove', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Remove Pilot Role</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained"> Remove Pilot Role</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
