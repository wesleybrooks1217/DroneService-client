...
const Takeover_drone = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('drones/takeover', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Takeover Drone</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained">Takeover Drone</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
