...
const Fire_employee = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('work_for/fire', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Fire Employee</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained">Fire Employee</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
