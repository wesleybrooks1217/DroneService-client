...
const Remove_drone = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('drones/remove', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Remove Drone</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained">Remove Drone</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
