...
const Add_owner = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('restaurant_owners/add', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Add Owner</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained">Add Owner</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
