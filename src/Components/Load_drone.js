...
const Load_drone = ({name}) => {
    ...
    const postEvent = () => {
      axios.post('payload/add', data).then(res => handleClose()).catch(err => console.log(err))
    }


    return (
        <div>
          <Button onClick={handleOpen} variant="contained">Load Drone</Button>
          <Modal
            open={open}
            onClose={handleClose}
          ...
          >
            <Box sx={style}>
                ...
                <TextField 
                  ...
                  sx={{ width:400 }}
                  value = {price}
                  onChange = {getDataPrice}
                  />
                </Grid>
                <Grid item xs={12}>
                <Button xs={12} onClick= {postEvent} variant="contained">Load Drone</Button>
                </Grid>
                </Grid>
            </Box>
          </Modal>
