...
const Owner_view = () => {
    ...
    const getAllEvents = () => {
        ...
        axios.get('display_pilot_view/all').then(response => {
            console.log(response)
            setEvent(response.data)
        })
        .catch (err => {
            console.log(err)
            alert("Error fetching Pilot information")
        })
    }
                    ...
                    <Grid item key={event.e_id} xs={12} sm={6} md={4}>
                        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                            <CardContent sx={{ flexGrow: 1 }}>
                                ...
                                <Typography variant = "h6">Experience = {event.experience}</Typography>
                                <Typography variant = "h6">Num Drones = {event.num_drones}</Typography>
                                <Typography variant = "h6">Num Locations = {event.num_locations}</Typography>
                            </CardContent>
                            <CardActions>
