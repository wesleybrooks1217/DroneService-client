...
const Owner_view = () => {
    ...
    const getAllEvents = () => {
        ...
        axios.get('display_ingredient_view/all').then(response => {
            console.log(response)
            setEvent(response.data)
        })
        .catch (err => {
            console.log(err)
            alert("Error fetching Ingredient information")
        })
    }
                    ...
                    <Grid item key={event.e_id} xs={12} sm={6} md={4}>
                        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                            <CardContent sx={{ flexGrow: 1 }}>
                                ...
                                <Typography variant = "h3">Ingredient Name = {event.iname}</Typography>
                                <Typography variant = "h6">Location = {event.location}</Typography>
                                <Typography variant = "h6">Amount Available = {event.amount_available}</Typography>
                                <Typography variant = "h6">Minp = {event.minp}</Typography>
                                <Typography variant = "h6">Maxp = {event.maxp}</Typography>
                            </CardContent>
                            <CardActions>
